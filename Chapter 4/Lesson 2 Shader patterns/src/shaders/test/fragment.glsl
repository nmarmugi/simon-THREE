...
void main()
{
    ...
    // gl_FragColor = vec4(vec3(strength), 1.0); // Pattern 17
    ...
    // gl_FragColor = vec4(vec3(strength), 1.0); // Pattern 19

    // float strength = step(0.4, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));

    // gl_FragColor = vec4(vec3(strength), 1.0); // Pattern 20
    ...
    // Fix only for pattern 11 - 14 - 15 colored version
